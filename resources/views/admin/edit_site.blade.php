...
<div class="col-sm-12 text-center" >
    ...
    <div class="d-flex">
      ...
      <div class="col-sm-6 text-center offset-sm-3" style="padding-top: 50px;">
      ...
      <div class="text-center" style="padding-bottom: 20px;">
        ...
        <div class="tagForm">
            <ul id="tagList_db">
                @foreach ($tags as $tag)
                    <li class="tags" id="id_{{$tag->id}}"> <a href="{{url('/select_categories_sites', $tag->tag_id)}}" > {{$tag->name}}</a>  @csrf <button class="deleteTagExsit btn-delete" id="id_{{$tag->id}}">X</button></li>
                @endforeach
            </ul>
        </div>
    ...
    $("#tagList_db").on('click', 'button.deleteTagExsit', function() {
        ...
        $.ajax({
            type:'POST',
            url:"{{url('/edit_films_ajax_delete_tag_sites')}}",
        ...
        <div class="tagForm">
            <ul id="tagList_db_films">
                @foreach ($tags_films as $tag)
                    <li class="tags" id="id_{{$tag->id}}"> <a href="{{url('/select_categories_sites_db_films', $tag->tag_id)}}" > {{$tag->name}}</a>  @csrf <button class="deleteTagExsit btn-delete" id="id_{{$tag->id}}">X</button></li>
                @endforeach
            </ul>
        </div>
    ...
    $("#tagList_db_films").on('click', 'button.deleteTagExsit', function() {
      ...
      <form action="{{url('/sites_tag_add_edit_site')}}" method="POST" enctype="multipart/form-data" id="add_films">
    @csrf <!-- {{ csrf_field() }} -->
      ...
      <div class="form-group">
        ...
        <form action="{{url('/edit_site_save')}}" method="POST" enctype="multipart/form-data" id="add_films">
          ...
          @csrf <!-- {{ csrf_field() }} -->
                    ...
                    <div class="form-group">
                      <textarea type="textarea" class="form-control @error('site_description') form-error @enderror" name="site_description" placeholder="Opis">{{$site->description}}</textarea>
          ...
          <div class="form-group">
        <label for="rating">Ocena Strony: {{$site->rating}}</label><br>
