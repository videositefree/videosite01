...
<div class="conatainer" style="padding-top: 10px; padding-bottom: 20px;">
    ...
    <div>
      <div>
        ...
        <img src="{{URL::asset("$stars->thumbnail")}}" style="height: 350px; width:250px;"></img>     
      </div>
...
<div class="text-center" style="padding-bottom: 20px;">
        ...
        <div class="tagForm">
            <ul id="tagList_db">
                @foreach ($tags as $tag)
                    <li class="tags" id="id_{{$tag->id}}"> <a href="{{url('/select_categories_stars', $tag->tag_id)}}" > {{$tag->name}}</a>  @csrf <button class="deleteTagExsit btn-delete" id="id_{{$tag->id}}">X</button></li>
                @endforeach
            </ul>
        </div>
    ...
    $("#tagList_db").on('click', 'button.deleteTagExsit', function() {
        ...
        $.ajax({
            type:'POST',
            url:"{{url('/edit_films_ajax_delete_tag_stars')}}",
...
<div class="text-center" style="padding-bottom: 20px;">
        ...
        filmów wystarczy przypisać gwiazdę do filmu
        </small>
        ...
        <div class="tagForm">
            <ul id="tagList_db_films">
                @foreach ($tags_films as $tag)
                    <li class="tags" id="id_{{$tag->id}}"> <a href="{{url('/select_categories_stars_db_films', $tag->tag_id)}}" > {{$tag->name}}</a>  @csrf <button class="deleteTagExsit btn-delete" id="id_{{$tag->id}}">X</button></li>
                @endforeach
            </ul>
        </div>
    ...
    $("#tagList_db_films").on('click', 'button.deleteTagExsit', function() {
        ...
        $.ajax({
            type:'POST',
            url:"{{url('/edit_films_ajax_delete_tag_stars_films')}}",
            data:'delete_id='+toDel,
        });
    ...
    <form action="{{url('/stars_tag_add_edit_site')}}" method="POST" enctype="multipart/form-data" id="add_films">
    @csrf <!-- {{ csrf_field() }} -->
      ...
      <div class="form-group">
        ...
        <form action="{{url('/edit_stars_save')}}" method="POST" enctype="multipart/form-data" id="add_films">
          ...
          <div class="form-group">
            <div class="custom-file mb-3">
              ...
              <label class="custom-file-label" for="thumbnail_stars" style="text-align: left">Wybierz Zdjęcie</label>
            </div>
          </div>
            
          <a class="btn btn-outline-secondary fas fa-folder-open" href="{{url('/open_folder_stars', $stars->id)}}" style="margin-bottom: 20px;"> Główny folder z gwiazdami</a>
   
          <div class="form-group">
            <div class="input-group"> 
              <input type="text" class="form-control @error('stars_name') form-error @enderror" name="stars_name" placeholder="Imie i nazwisko Gwiazdy" value="{{$stars->name}}">
              <div class="input-group-append">
                <a class="btn btn-outline-secondary fas fa-folder-open" href="{{url('/open_folder_stars_next', $stars->id)}}" ></a>   
              </div>
              @error('stars_name')
          ...
          <div class="form-group">
        <label for="rating">Ocena Gwiazdy: {{$stars->rating}}</label><br>
