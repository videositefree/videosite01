...
@section('title')VideoSite Edytuj Wytwórnie @endsection

@section('direction')
<ol class="breadcrumb">
    <li class="breadcrumb-item"><a href="{{ url('/admin_studios') }}">Wytwórnie</a></li>
    <li class="breadcrumb-item active" aria-current="page">Edytowanie Wytwórni</li>
</ol>
@endsection
...
<div class="upload-page">
    ...
    <h1 class="upload-page__title">Edytuj: {{$studios->name}}</h1>
        ...
        <div class="upload-preview">
            <div class="upload-preview__card">
                ...
                <div class="resizable-video" style="aspect-ratio:1/1; margin-bottom:16px;">
                    <img src="{{URL::asset("$studios->thumbnail")}}" style="width:100%; height:100%; object-fit:contain;">
                </div>
        ...
        <div class="upload-form-wrap">
            ...
            <section class="upload-section">
                        ...
                        <ul class="upload-pill-list" id="tagList_db">
                            @foreach ($tags as $tag)
                                <li class="tags" id="id_{{$tag->id}}">
                                    <a href="{{url('/select_categories_studios', $tag->tag_id)}}" target="_blank">{{$tag->name}}</a>
                ...
                <div class="upload-subgroup">
                    ...
                    <small class="form-text text-muted">Korzystając z tagów filmowych, dodasz je automatycznie do filmów, wystarczy przypisać wytwórnię do filmu.</small>
                        ...
                        <ul class="upload-pill-list" id="tagList_db_films" style="margin-top:10px;">
                            @foreach ($tags_films as $tag)
                                <li class="tags" id="filmid_{{$tag->id}}">
                                    <a href="{{url('/select_categories_studios_db_films', $tag->tag_id)}}" target="_blank">{{$tag->name}}</a>
                ...
                <form action="{{url('/studios_tag_add_edit_site')}}" method="POST" enctype="multipart/form-data" id="add_tags_studios">
                    @csrf
                    <input value="{{$studios->id}}" name="id" hidden>

                    <div class="upload-subgroup">
                        <label class="upload-subgroup__label">Dodaj tagi gwiazd</label>
                        <div class="input-group">
                            <input type="text" class="form-control ui-autocomplete-input" id="tag" placeholder="Dodaj nowe tagi wytwórni" autocomplete="off">
            ...
            <section class="upload-section">
                ...
                <form action="{{url('/edit_studios_save')}}" method="POST" enctype="multipart/form-data" id="edit_studios_main">
                    @csrf

                    <div class="custom-file mb-3">
                        <input type="file" class="custom-file-input" id="thumbnail_tags" name="thumbnail_studios">
                        <label class="custom-file-label" for="thumbnail_studios">Wybierz zdjęcie</label>
                    </div>

                    <a class="admin-quicklink" href="{{url('/open_folder_studios', $studios->id)}}" style="margin-bottom: 16px; display:inline-flex;">
                        ...
                        Główny folder z wytwórniami
                    </a>

                    <div class="form-group">
                        <div class="input-group">
                            <input type="text" class="form-control @error('studios_name') form-error @enderror" name="studios_name" placeholder="Nazwa wytwórni" value="{{$studios->name}}">
                            <div class="input-group-append">
                                <a class="btn btn-outline-secondary fas fa-folder-open" href="{{url('/open_folder_studios_next', $studios->id)}}" title="Otwórz folder"></a>
                            </div>
                        </div>
                        @error('studios_name')<div class="alert alert-danger valid_msg">{{ $message }}</div>@enderror
                    </div>

                    <input type="hidden" value="{{$studios->id}}" name="studios_id">

                    <div class="form-group">
                        <label>Ocena wytwórni: <b>{{$studios->rating}}</b></label>
                        <div class="upload-toggle-row" style="flex-wrap:wrap; gap:14px;">
                            @for ($i = 1; $i <= 6; $i++)
                                <label style="margin:0;">
                                    <input type="radio" id="rating_{{ $i }}" name="rating" value="{{ $i }}" {{ $studios->rating == $i ? 'checked' : '' }}> {{ $i }}
                                </label>
                            @endfor
                        </div>
                        @error('rating')<div class="alert alert-danger valid_msg">{{ $message }}</div>@enderror
                    </div>
                    <input type="hidden" value="{{$studios->rating}}" name="hidden_rating">
...
$(function() {
    ...
    $("#tagList_db").on('click', 'button.deleteTagExsit', function() {
        ...
        $("#id_"+toDel).remove();
        $.ajax({ type: 'POST', url: "{{url('/edit_films_ajax_delete_tag_studios')}}", data: 'delete_id='+toDel });
    });
