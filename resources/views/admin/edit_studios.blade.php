...
<div class="conatainer" style="padding-top: 10px; padding-bottom: 20px;">
    ...
    <div>
      <div>
        ...
        <img src="{{URL::asset("$studios->thumbnail")}}" style="height: 350px; width:250px;"></img>     
      </div>
...
<div class="text-center" style="padding-bottom: 20px;">
        <div class="" style="padding-bottom: 1%; padding-left: 2%"> Tagi Dedykowane </div>
        @if(count($tags) > 0)
        ...
        <div class="tagForm">
            <ul id="tagList_db">
                @foreach ($tags as $tag)
                    <li class="tags" id="id_{{$tag->id}}"> <a href="{{url('/select_categories_studios', $tag->tag_id)}}" > {{$tag->name}}</a>  @csrf <button class="deleteTagExsit btn-delete" id="id_{{$tag->id}}">X</button></li>
                @endforeach
            </ul>
        </div>
    ...
    $("#tagList_db").on('click', 'button.deleteTagExsit', function() {
        ...
        $.ajax({
            type:'POST',
            url:"{{url('/edit_films_ajax_delete_tag_studios')}}",
    ...
    <div class="text-center" style="padding-bottom: 20px;">
        <div class="" style="padding-bottom: 1%; padding-left: 2%"> Tagi Filmowe</div>
        ...
        <div class="tagForm">
            <ul id="tagList_db_films">
                @foreach ($tags_films as $tag)
                    <li class="tags" id="id_{{$tag->id}}"> <a href="{{url('/select_categories_studios_db_films', $tag->tag_id)}}" > {{$tag->name}}</a>  @csrf <button class="deleteTagExsit btn-delete" id="id_{{$tag->id}}">X</button></li>
                @endforeach
            </ul>
        </div>
    ...
    <form action="{{url('/studios_tag_add_edit_site')}}" method="POST" enctype="multipart/form-data" id="add_films">
    @csrf <!-- {{ csrf_field() }} -->
      ...
      <div class="form-group">
        ...
        <form action="{{url('/edit_studios_save')}}" method="POST" enctype="multipart/form-data" id="add_films">
          ...
          <div class="form-group">
            <div class="custom-file mb-3">
              ...
              <label class="custom-file-label" for="thumbnail_studios" style="text-align: left">Wybierz Zdjęcie</label>
            </div>
          </div>
            
          <a class="btn btn-outline-secondary fas fa-folder-open" href="{{url('/open_folder_studios', $studios->id)}}" style="margin-bottom: 20px;"> Główny folder z wytwórniami</a>

          <div class="form-group">
            <div class="input-group"> 
              <input type="text" class="form-control @error('studios_name') form-error @enderror" name="studios_name" placeholder="Nazwa Tagu" value="{{$studios->name}}">
              <div class="input-group-append">
                <a class="btn btn-outline-secondary fas fa-folder-open" href="{{url('/open_folder_studios_next', $studios->id)}}" ></a>   
              </div>
              @error('studios_name')
          ...
          <div class="form-group">
        <label for="rating">Ocena Wytwórni: {{$studios->rating}}</label><br>
